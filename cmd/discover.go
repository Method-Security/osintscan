...
import (
	...
	takeover "github.com/Method-Security/osintscan/internal/discover/dns/takeover"
...
func (a *OsintScan) InitDiscoverCommand() {
	...
	// Takeover Command
	discoverDNSTakeoverCmd := &cobra.Command{
		Use:   "takeover",
...
infrastructure the target still controls.`,
	...
	// Target Flags
	discoverDNSTakeoverCmd.Flags().StringSlice("targets", []string{}, "A list of URLs or domains to analyze for takeover vulnerabilities")
	discoverDNSTakeoverCmd.Flags().StringSlice("target-files", []string{}, "File paths containing lists of targets to analyze for takeover vulnerabilities")
	// Config Flags
	discoverDNSTakeoverCmd.Flags().String("fingerprints-file", "", "Path to the JSON file containing service fingerprints for takeover detection")
	discoverDNSTakeoverCmd.Flags().Bool("successful-only", false, "Show only confirmed successful takeovers in the results")
	discoverDNSTakeoverCmd.Flags().Bool("verify-tls", false, "Verify TLS certificates when making HTTPS requests during takeover analysis")
	discoverDNSTakeoverCmd.Flags().Int("timeout", 180, "Timeout in seconds for each takeover check request")

	// Mark Required Flags
	_ = discoverDNSTakeoverCmd.MarkFlagRequired("targets")

	// Add command to 'dns' command
	discoverDNSCmd.AddCommand(discoverDNSTakeoverCmd)
...
func getDNSTakeoverConfig(targets []string, targetFiles []string, fingerprintsFile string, successfulOnly bool, verifyTLS bool, timeout int) dnsfern.DiscoverDomainTakeoverConfig {
	config := dnsfern.DiscoverDomainTakeoverConfig{
		Targets:        targets,
